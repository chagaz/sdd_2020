...
\paragraph{Preuve}
Considérons un vecteur $\ww \in \RR^p$. La projection de de $X$ sur $\ww$ est
...
\subsection{Décomposition en valeurs singulières $\bigstar$}
\paragraph{Proposition} 
...
La matrice $H \in \RR^{n \times m}$ peut être interprétée comme une
...
Elle correspond en effet à considérer que les données sont les réalisation d'un
vecteur aléatoire $(X_1, X_2, \dots, X_p)$ obtenues par
% \begin{equation}
%   \label{eq:fa_model}
...
\item Il existe de nombreuses approches de réduction de dimension
  ...
  composantes linéaires des variables initiales. Parmi elles, citons 
  \begin{itemize}

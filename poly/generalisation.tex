...
Le théorème du {\it no free lunch} de~\citet{wolpert1997} indique qu'aucun
...
une technique que nous verrons plus en détail au cours de cet ouvrage (à
commencer par le chapitre~\ref{chap:regularisation}), et qui consiste à ajouter
au terme d'erreur que l'on cherche à minimiser un terme qui mesure la
complexité du problème (par exemple, dans le cas précédent, le degré du
polynôme ou le nombre de coefficients du modèle). Ainsi, un modèle complexe qui
a une erreur empirique faible peut être défavorisé face à une modèle plus
simple, même si celui-ci présente une erreur empirique plus élevée.
...
\subsection{Solution}
Le problème~\ref{eq:ridgereg} est un problème d'optimisation convexe (voir
section~\ref{sec:optimisation}) : il s'agit de minimiser une forme
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{figures/generalisation/ridge_path}
...
ayant un coefficient nul, le lasso est une méthode de sélection de variable
...
un algorithme à directions de descente (voir
section~\ref{sec:gradient_descent}) pour le résoudre. De plus, il ne s'agit pas
...
\begin{figure}[h]
  \centering \includegraphics[width=0.7\textwidth]{figures/generalisation/l1reg}
  \caption{La solution du problème d'optimisation sous
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{figures/generalisation/lasso_path}

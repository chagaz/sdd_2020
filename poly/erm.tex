%-*- coding: iso-latin-1 -*-
\label{chap:erm}
Dans cette partie, nous nous intéresserons principalement aux problèmes
d'apprentissage {\it supervisé} : il s'agit de développer des algorithmes qui
soient capables d'apprendre des modèles {\it prédictifs}. À partir d'exemples
étiquetés, ces modèles seront capables de prédire l'étiquette de nouveaux
objets. Le but de ce chapitre est de développer les concepts généraux qui nous
permettent de formaliser ce type de problèmes.
...
variables sont aussi appelées {\it descripteurs}, {\it attributs}, {\it
  ...
  points du jeu de donnée} (en anglais, {\it samples} ou {\it data
...
% Dans l'exemple de la figure~\ref{fig:simple_classif_pb}, on pourra décider de se
% restreindre à des discriminants qui soient des ellipses à axes parallèles aux axes de coordonnées. 
% Ainsi, l'espace des hypothèse sera 
% \begin{equation*}
...
Dans le cadre d'un problème d'apprentissage supervisé, on appelle {\it
  risque} l'espérance d'une fonction de coût (voir 
chapitre~\ref{chap:bayes} pour la modélisation probabiliste d'un problème
...
problème d'optimisation (voir l'annexe~\ref{sec:optimisation} sur
...
\todo{Detail here, link to estimation earlier on}
...
la théorie de l'apprentissage de Vapnik-Chervonenkis, qui dépasse l'ambition de
cet ouvrage.
...
\subsubsection{Coût 0/1 pour la classification binaire}
...
\todo{Transition here using the intro to the regression chapter.}
...
(cf. section~\ref{sec:bayes_param_model}).
...
estimation de $\bbeta$ par un algorithme à directions de descente (voir la
section~\ref{sec:gradient_descent}).

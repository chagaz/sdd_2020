...
$\epsilon$ telle que
  \begin{equation}
  Y = \Phi(X) + \epsilon,\label{eq:probabilistic_ml}
\end{equation}
...
formalisé par la notion de \textbf{risque.} En supposant que les couples
$(\xx^i, y^i)$ soient les réalisations de $n$ vecteurs aléatoires de même loi
...
On parle de \textbf{modèle paramétrique} quand on utilise un algorithme
...
appelle la \textbf{minimisation des moindres carrés}, une méthode bien connue
...
Commençons par considérer des modèles \textbf{linéaires}: nous cherchons à
expliquer la variable cible $y$ par une \textbf{combinaison linéaire} -- en
...
On fera attention à ne pas confondre les \textbf{variables}, qui sont les $p$
valeurs $x_1, x_2, \dots, x_p$ qui décrivent les données, et les
\textbf{paramètres}, qui sont les $p+1$ valeurs
$\beta_0, \beta_1, \dots, \beta_p$ qui paramètrent le modèle.

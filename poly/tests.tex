...
\item « Deux variables statistique $X$ et $Y$ sont indépendantes » (c'est le
  cas du test d'indépendance du $\chi^2$, \todo{cf. TD}).
...
moyenne empirique $\hat{m}$ de 312g si l'espérance de $X$ est de 300g.  En
...
l'estimer grâce à l'écart-type empirique $\hat{\sigma} = 31\si{g},$ et utiliser 
\begin{equation}
  \frac{\sqrt{n} (M_n - \mu)}{\hat \sigma}  \sim \Ncal(0, 1).
...
\paragraph{Question :} Comment utiliser l'équation~\ref{eq:tcl_moyenne} pour
...
\begin{answer}
  ...
  \begin{equation}
    \label{eq:z_moyennes}
    Z = \frac{\sqrt{n} (M_n - \mu_0)}{\hat \sigma}
  \end{equation}
  est une gaussienne standard : $Z \sim \Ncal(0, 1)$.

  $Z$ est donc une variable aléatoire rélle dont nous connaissons la
...
une valeur prédéterminé consiste donc à rejeter $\HH_0$ si $\abs{Z} > z_0.$
...
sandwhicheries environnantes, ne seraient pas plus lourds que la moyenne de
...
D'après la définition de $Z$ (équation~\ref{eq:z_moyennes}), cela est équivalent à 
\[
  \PP\left( M_n - \frac{\hat{\sigma}}{\sqrt{n}} z_0 \leq 
    \mu \leq M_n + \frac{\hat{\sigma}}{\sqrt{n}}z_0 \right) = 1 - \alpha.
\]

Ainsi l'intervalle 
\[
  \left[ M_n - \frac{\hat{\sigma}}{\sqrt{n}} z_0, \; M_n +
    \frac{\hat{\sigma}}{\sqrt{n}} z_0 \right]
\]
...
le test d'hypothèse unilatéral à droite.
\todo{Rédiger la solution}
...
\begin{plusloin}
\item \todo{On dit d'un test statistique qu'il est sans biais si}
\item \todo{On dit d'un test statistique qu'il converge si}
\item \todo{Si la distribution de la statistique de test sous l'hypothèse nulle n'est
  pas connue, on peut recourir à des méthodes empiriques pour la déterminer de
  manière empirique.}

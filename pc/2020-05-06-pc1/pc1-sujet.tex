...
} la proposition suivante : Soient deux variables aléatoires réelles
$X$ et $Y$ indépendantes, ayant respectivement chacune $K$ et $L$ modes. Posons
$d_{\chi^2}$ leur distance du chi2. Soit $n$ la taille d'un échantillon
aléatoire de $(X, Y)$ et $d_{\chi^2}$ la distance du chi2 de la table de
contingence de cet échantillon. Alors quand $n \rightarrow +\infty,$
\[
...
\item \textbf{Test du chi2}
  ...
  \item \textbf{Fraude scientifique.} À un niveau de
    signification de $5\%$, combien de personnes traitées faudrait-il trouver
    une bonne raison d'exclure de l'étude afin de pouvoir rejeter l'hypothèse

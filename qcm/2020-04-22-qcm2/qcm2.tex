...
Les deux sont des estimateurs sans biais de $\lambda$.

En effet, les $X_i$ étant i.i.d. de même loi que $X,$ et on rappelle que
$\EE(X) = \lambda$ et $\VV(X) = \lambda.$ Alors
\[
  \text{B}(L_2) = \EE(L_1) - \lambda = \frac1n \sum_{i=1}^n \EE(X_i) - \lambda = 0.
\]

De même, 
\[
  \text{B}(L_3) = \frac1n \sum_{i=1}^n \left( \EE(X_i)^2 - \lambda^2 \right)  - \lambda = 0.
\]
On a utilisé ici que, par définition de la variance, $\EE(X^2) = \VV(X) + \EE(X)^2.$
